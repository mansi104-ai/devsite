...
Before diving into the need for data augmentation, let's explore a basic neural network and a Convolutional Neural Network (CNN) using identical parameters:
...
Just take a look at the number of trainable param from the above data and we'll see what we can infer from them.
